...
现在定义一个矢量$\pmb{\omega}$，它的大小等于式\eqref{eqn:01.09.01}中的$\omega$，$\pmb{\omega}$的
...
\noindent 当我们从$\pmb{\omega}$指向的方向观察质点的运动时，质点总是沿逆时钟方
向转动。这个规定$\pmb{\omega}$的方向的方法也可以用一个正扣螺旋来说
明，当螺钉按质点运动方向旋转时，螺钉的运动方向就是$\pmb{\omega}$的方
向。

这样定义的$\pmb{\omega}$，称为角速度矢量。利用这个矢量。可以把质
点的速度矢量$\vec{v}$表示成
\begin{equation}\label{eqn:01.09.04}
    \vec{v}=\pmb{\omega}\times \vec{r}
\end{equation}
由图\ref{fig:01.20}，因为$\pmb{\omega}$与$\vec{r}$垂直，所以$|\pmb{\omega}\times\vec{r}|=\omega r=v$，
而且$\pmb{\omega}\times\vec{r}$的方向与$\vec{v}$的方向一致。故式\eqref{eqn:01.09.04}
...
\begin{figure}[!h]
    ...
    \begin{minipage}[b]{15em}
        ...
        \vspace{4em}
        \caption{圆周运动中$\pmb{\omega}$与$\vec{v}$的关系}
        \label{fig:01.20}
    \end{minipage}
    \hfill
